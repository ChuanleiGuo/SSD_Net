...
class RollingDetector(object):
    ...
    def detect(self, det_iter, show_timer=False):
        ...
        for idx, detection in enumerate(detections):
            ...
            for output in detection:
                ...
                #     r.append(res)
                res = output[np.where(output[:, 0] >= 0)]
                r.append(res)
            result.append(r)
        return result
    ...
    def im_detect(self, im_list, root_dir=None, extension=None, show_timer=False):
        ...
        test_iter = DetIter(test_db, 1, self.data_shape[1:], self.mean_pixels, is_train=False)
            ...
            if cls_id >= 0:
                ...
                if score > thresh:
                    ...
                    rect = plt.Rectangle((xmin, ymin), xmax - xmin,
                        ymax - ymin, fill=False, edgecolor=colors[cls_id],
                        linewidth=3.5)
    ...
    def detect_and_visualize(self, im_list, root_dir=None, extension=None,
        ...
        im_list = [os.path.join(root_dir, im) + extension for im in im_list]
        for k, det in enumerate(dets):
            img = cv2.imread(im_list[k])
            img[:, :, (0, 1, 2)] = img[:, :, (2, 1, 0)]
            self.visualize_detection(img, det[4], classes, thresh)
