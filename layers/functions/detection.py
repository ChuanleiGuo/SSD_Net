...
class Detect(Function):
    ...
    and threshold to a top_k number of output predictions for both
    ...
    def __init__(self, num_classes, bkg_label, top_k, conf_thresh, nms_thresh):
        ...
        if nms_thresh <= 0:
            raise ValueError("nms_threshold muse be non negative.")
    ...
    def forward(self, loc_data, conf_data, prior_data):
        """
        args:
            loc_data: (tensor) Loc preds from loc layers, [batch, num_priors * 4]
            conf_data: (tensor) Conf preds from conf layers, [batch * num_priors, num_classes]
            prior_data: (tensor) Prior boxes and variances from priorbox layers, [1, num_priors, 4]
        """
        num = loc_data.size(0)
