...
class MApMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        for i in range(labels[0].shape[0]):
            ...
            while (pred.shape[0] > 0):
                ...
                records = np.hstack((dets[:, 1][:, np.newaxis], np.zeros(
                    (dets.shape[0], 1))))
                # ground-truths
    ...
    def _update(self):
        ...
        for k, v in self.records.items():
            ...
            if self.roc_output_path is not None:
                self.save_roc_graph(recall=recall, prec=prec, classkey=k, \
                    path=self.roc_output_path, ap=ap)
            aps.append(ap)
    ...
    def _recall_prec(self, record, count):
        ...
        record = np.delete(
            record, np.where(record[:, 1].astype(int) == 0)[0], axis=0)
