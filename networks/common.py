...
def multi_layer_feature(body, from_layers, num_filters, strides, pads, min_filter=128):
    ...
    # gets a new grouped symbol sgroup. The output of sgroup is
    # a list of outputs of all of the internal nodes.
    # doc: http://mxnet.incubator.apache.org/api/python/symbol.html#mxnet.symbol.Symbol.get_internals
...
def multibox_layer(from_layers, num_classes, sizes=[.2, .95],
    ...
    normalization: int or list of int
        use normalization value for all layers or [...] for specific layers,
        -1 indicate no normalization and scales
    ...
    if len(sizes) == 2 and not isinstance(sizes[0], list):
        ...
        assert 0 < sizes[0] < 1
        assert 0 < sizes[1] < 1 and sizes[1] > sizes[0]
    ...
    if steps:
        assert len(steps) == len(from_layers), \
            "provides steps for all layers or leave empty"
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        if interm_layer > 0:
            from_layer = mx.symbol.Convolution(
            ...
            from_layer = mx.symbol.Activation(
                data=from_layer, act_type="relu", name="{}_inter_relu".format(from_name))
        
        # estimate number of anchor per location
        size = sizes[k]
        ...
        bias = mx.symbol.Variable(name="{}_loc_pred_conv_bias".format(from_name),
            init=mx.init.Constant(0.0), lr_mult=2.0)
        ...
        if steps:
            ...
            step = '(-1.0 -1.0)'
        anchors = mx.contrib.symbol.MultiBoxPrior(data=from_layer, sizes=size_str, ratios=ratio_str,
    ...
    cls_preds = mx.symbol.transpose(data=cls_preds, axes=(0, 2, 1), name="multibox_cls_pred")
