...
def multibox_layer(from_layers,
    ...
    if steps:
        assert len(steps) == len(
            from_layers), "provide steps for all layers or leave empty"
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        bias = mx.symbol.Variable(
            name="{}_loc_pred_conv_bias".format(from_name),
            init=mx.init.Constant(0.0),
            attr={
                '__lr_mult__': '2.0'
            })
        ...
        bias = mx.symbol.Variable(
            name="{}_cls_pred_conv_bias".format(from_name),
            init=mx.init.Constant(0.0),
            attr={
                '__lr_mult__': '2.0'
            })
