...
def get_symbol_train(num_classes=20, nms_thresh=0.5, force_suppress=False,
                     nms_topk=400, **kwargs):
    """
    Single-shot multibox detection with VGG16 layers ConvNet
    This is a modified version, with fc6/fc7 layers replaced by conv layers
    And the network is slightly smaller than original VGG16 network
    ...
    force_suppress: boolean
        whether suppress diferent class objects
    nms_topk: int
    ...
    conv6 = mx.symbol.Convolution(
        data=pool5, kernel=(3, 3), pad=(6, 6), dilate=(6, 6), num_filter=1024, name="conv6")
    ...
    # specific parameters from VGG16 network
    ...
    loc_preds, cls_preds, anchor_boxes = multibox_layer(from_layers, num_classes,
        sizes=sizes, ratios=ratios, normalization=normalizations,
        num_channels=num_channels, clip=False, interm_layer=0, steps=steps)

    tmp = mx.contrib.symbol.MultiBoxTarget(anchor=anchor_boxes, label=label,
        cls_pred=cls_preds, overlap_threshold=.5, ignore_label=-1, negative_mining_ratio=3,
        minimum_negative_samples=0, negative_mining_thresh=.5, variances=(0.1, 0.1, 0.2, 0.2),
...
def get_symbol(num_classes=20, nms_thresh=0.5, force_suppress=False,
    ...
    cls_prob = mx.symbol.SoftmaxActivation(data=cls_preds, mode="channel",
        name="cls_prob")
    out = mx.contrib.symbol.MultiBoxDetection(cls_prob=cls_prob, loc_pred=loc_preds,
        anchor=anchor_boxes, name="detection", nms_threshold=nms_thresh,
        force_suppress=force_suppress, variances=(0.1, 0.1, 0.2, 0.2), nms_topk=nms_topk)
    return out
