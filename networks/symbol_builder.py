...
def get_symbol_train(network, num_classes, from_layers, num_filters, strides, pads,
                     sizes, ratios, normalization=-1, steps=[], min_filter=128,
    ...
    from_layers: list of str
        feature extraction layers, use '' for adding extra layers
        For example:
    ...
    loc_preds, cls_preds, anchor_boxes = multibox_layer(
        layers, num_classes, sizes=sizes, ratios=ratios, normalization=normalization,
        num_channels=num_filters, clip=False, interm_layer=0, steps=steps)

    tmp = mx.contrib.symbol.MultiBoxTarget(
        *[anchor_boxes, label, cls_preds], overlap_threshold=.5, ignore_label=-1,
        negative_mining_ratio=3, minimum_negative_samples=0, negative_mining_thresh=.5,
